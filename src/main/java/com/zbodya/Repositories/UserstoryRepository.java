...
public interface UserstoryRepository extends PagingAndSortingRepository<Userstory, Long> 
{
	...
	@Query("select sum(u.countStoryPoints) from Userstory u join u.sprints s where u.status='Done' and s.id=?1 group by u.status ")
	Integer getStorypointsCountFromDoneUserstoriesBySprintID(Long id);
